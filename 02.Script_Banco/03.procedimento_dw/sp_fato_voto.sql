...
AS BEGIN
	  ...
	  @COD_LOCALIDADE_ELEICAO INT, @ID_LOCALIDADE INT, @ID_CANDIDATO INT, @ID_ELEITOR INT, @ID_SESSAO INT, @ID_CARGO INT, @ID_TIPO_VOTO INT,
	  ...
	  BEGIN
	       IF (EXISTS (SELECT 1 FROM DIM_LOCALIDADE L WHERE @COD_CIDADE = L.COD_CIDADE AND @COD_ESTADO = L.COD_ESTADO) AND
			  ...
			  EXISTS (SELECT 1 FROM DIM_LOCALIDADE_ELEICAO LE WHERE @COD_LOCALIDADE_ELEICAO = LE.COD_LOCALIDADE_ELEICAO)
			  )       
			  BEGIN
